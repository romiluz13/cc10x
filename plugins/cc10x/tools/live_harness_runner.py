...
def fail(message: str, *, exit_code: int = 3) -> int:
    print(
        json.dumps({"status": "INVALID", "error": message}, indent=2), file=sys.stderr
    )
    return exit_code
...
def filter_scenarios(
    scenarios: list[dict[str, Any]], mode: str
) -> list[dict[str, Any]]:
...
def main() -> int:
    ...
    parser.add_argument(
        "--manifest", required=True, help="Path to the harness manifest JSON"
    )
    parser.add_argument(
        "--mode",
    ...
    for category, steps in sequence:
        ...
        for step in steps:
            row = run_step(
                step, category=category, workdir=workdir, base_env=manifest_env
            )
            rows.append(row)
            if row["status"] != "PASS":
    ...
    for step in cleanup_steps:
        rows.append(
            run_step(step, category="cleanup", workdir=workdir, base_env=manifest_env)
        )
