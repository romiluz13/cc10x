...
description: Deployment and rollback guidance for production releases. Provides phased rollout templates, monitoring checklists, and recovery playbooks. Used by the planning workflow and planner subagent (which consolidates planning-design-deployment).
...
- Commands invoke sub-agents (e.g., `planner`, which consolidates planning-design-deployment)
...
```
# In planner subagent (consolidates planning-design-deployment):
