...
```typescript
// ‚úGood: Each function does one thing
...
```typescript
// ‚ùBad: Does too much, hard to test
...
```typescript
// ‚úGood: Specific error types, helpful messages
...
```typescript
// ‚ùBad: Generic errors, no context
...
‚îú‚îÄ‚îfeatures/
‚î  ‚îú‚îÄ‚îauth/
‚î  ‚î  ‚îú‚îÄ‚îauth.service.ts      # Business logic
‚î  ‚î  ‚îú‚îÄ‚îauth.controller.ts   # HTTP handlers
‚î  ‚î  ‚îú‚îÄ‚îauth.types.ts        # Types/interfaces
‚î  ‚î  ‚îî‚îÄ‚îauth.test.ts         # Tests
‚î  ‚îî‚îÄ‚îorders/
‚î      ‚îú‚îÄ‚îorders.service.ts
‚î      ‚îú‚îÄ‚îorders.controller.ts
‚î      ‚îú‚îÄ‚îorders.types.ts
‚î      ‚îî‚îÄ‚îorders.test.ts
‚îú‚îÄ‚îshared/
‚î  ‚îú‚îÄ‚îutils/
‚î  ‚îú‚îÄ‚îtypes/
‚î  ‚îî‚îÄ‚îerrors/
...
**In a project using**:
- Services for business logic ‚ÜCreate `feature.service.ts`
- Controllers for HTTP ‚ÜCreate `feature.controller.ts`
- Zod for validation ‚ÜUse Zod schemas
- Custom error classes ‚ÜCreate feature-specific errors
...
```typescript
// ‚úGood: Explains WHY (non-obvious reasoning)
...
```typescript
// ‚ùBad: Explains WHAT (code already shows)
...
```typescript
// ‚úGood: Clear, correct, fast enough
...
```typescript
// ‚ùBad: Over-optimized, hard to read
function findUsersByRole(users: User[], role: string): User[] {
  const result: User[] = [];
...
**Async operations**:
```typescript
// ‚úGood: Parallel independent operations
...
```typescript
// ‚ùBad: Sequential when could be parallel
...
```typescript
// ‚úGood: Early returns, flat structure
function processOrder(order: Order): ProcessedOrder {
  if (!order.items || order.items.length === 0) {
    throw new ValidationError('Order must contain items');
  }
...
**Good code is**:
- ‚úEasy to read and understand
- ‚úEasy to modify and extend
- ‚úEasy to test
- ‚úEasy to debug

**Good code is NOT**:
- ‚ùClever or impressive
- ‚ùUsing every language feature
- ‚ùOver-engineered for future needs
- ‚ùOptimized before profiling
