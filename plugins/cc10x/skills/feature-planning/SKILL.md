...
Phase 6: Plan Testing Strategy (quality assurance)
    ‚Üì
Output: Comprehensive Plan ‚ÜReady for /feature-build
```
...
- ‚ùSkipping requirements phase (leads to rework)
- ‚ùOver-engineering (YAGNI - You Aren't Gonna Need It)
- ‚ùIgnoring edge cases (they always happen in production)
- ‚ùNo testing strategy (bugs found late are expensive)
- ‚ùUnclear acceptance criteria (when is it "done"?)
- ‚ùMissing error handling (happy path only)
...
A great plan should:
- ‚úBe actionable (clear next steps)
- ‚úBe testable (clear acceptance criteria)
- ‚úBe realistic (achievable time estimates)
- ‚úAddress risks (identified and mitigated)
- ‚úBe documented (persistent markdown file)
- ‚úBe ready for `/feature-build` execution
