...
- [ ] Touch targets ‚â44√ó44px on mobile?
...
**Loading States**:
```typescript
// ‚ùNo loading state (users see blank screen)
function UserList() {
...
// ‚úWith loading state and skeleton
function UserList() {
  const [users, setUsers] = useState([]);
...
**Error Messages**:
```typescript
// ‚ùTechnical error (users confused)
catch (error) {
  alert(error.message); // "ERR_CONNECTION_REFUSED"
}

// ‚úUser-friendly error (users understand + can recover)
catch (error) {
  showNotification({
    type: 'error',
...
```typescript
// ‚ùValidation only on submit (frustrating!)
<form onSubmit={handleSubmit}>
  <input name="email" />
  {/* User fills entire form, clicks submit, sees all errors at once */}
</form>

// ‚úInline validation as user types
<input
  name="email"
...
| Small touch targets | Hard to tap on mobile | Minimum 44√ó44px |
