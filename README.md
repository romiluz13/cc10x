...
**What are MDC files?** Markdown with Context - `.mdc` files with YAML frontmatter for smart auto-loading

**Frontmatter properties**:
```yaml
---
description: What this context rule provides
alwaysApply: true    # Auto-load in every session
globs: "*.tsx"       # Auto-load when editing matching files (future)
---
```

**Used for**: Context rules that need dynamic loading based on file patterns

**Examples in cc10x**:
- `.claude/context/rules/project-status.mdc` (alwaysApply: true)
- `.claude/context/rules/coding-standards.mdc` (alwaysApply: true)

**Benefits**:
- **Smart auto-loading**: Essential context loads automatically
- **Glob-based triggering**: Future support for context switching based on file patterns
- **Token efficiency**: Only load relevant context (foundation for 93% savings)
- **Extensible**: Add custom rules with frontmatter metadata

**MDC vs MD decision tree**:
```
Is this a context rule?
├─ YES → Use MDC (.mdc) with frontmatter
...
### Hybrid Task Tracking (Best of Both Worlds)
**Two complementary systems working together**:

1. **Persistent Markdown Checklists** (`.claude/docs/checklist-[name].md`)
   - Long-term project planning (50+ tasks)
   - Organized by phases with deliverables and acceptance criteria
   - Version controlled, team-visible
   - Generated by `/feature-plan` command

2. **TodoWrite Tool** (Ephemeral)
   - Active development sessions (5-10 tasks)
   - Real-time UI progress updates
   - Current phase focus only
   - Cleared between sessions

**Workflow**:
```
/feature-plan → Generate persistent checklist (50 tasks, all phases)
/feature-build → Load current phase to TodoWrite (5 tasks)
During work → Mark TodoWrite as complete (real-time feedback)
After phase → Update persistent checklist (long-term tracking)
```

**When to use each**:
- ✅ Persistent checklist: Complex features, multi-session work, team collaboration
- ✅ TodoWrite: Active session tracking, real-time progress, simple features
- ✅ Both together: Best experience for long-term feature development
