...
1. **Doubt-Driven Development** — Fresh-context adversarial reviewer spawned IN-FLIGHT (not post-hoc) for non-trivial decisions. 5-step cycle: CLAIM → EXTRACT → DOUBT → RECONCILE → STOP. Cross-model escalation (Gemini/Codex as second opinion). 9-entry rationalization table. 12 red flags. Key insight: NOT `/review` (post-hoc verdict) — this is in-flight course correction while changes are still cheap. The reviewer gets ARTIFACT + CONTRACT only, NOT the CLAIM (prevents biasing toward agreement). 3-cycle cap before escalation. "Doubt theater" detection: if 2+ cycles produce substantive findings but zero are classified as actionable, you're validating not doubting. Cross-model is always offered in interactive mode (never silently skipped). This is the most innovative pattern found across all 6 repos.
